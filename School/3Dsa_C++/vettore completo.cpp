...
int main()
{
//	----MAIN VARIABLES DECLERETIONS----
	char risp;
	int dim, elemento, e2, posto;
	int vett[MAX];
	bool trovato;
//	----END OF VARIBLES DECLERETIONS----

do
{
	dim=chiedi_dimensione();
	carica(vett, dim);
	system("cls");
	
// ------RICERCA SEQUENZIALE MAIN-------

cout<<"Inserisci elemeto da cercare: ";
cin>>elemento;
// ----RICERCA SEQUENZIALE FIRST FUNCTION-----	
trovato=ricerca_sequenziale1(vett, dim, elemento, posto);
if(trovato)
	cout<<"L'elemento "<<elemento<<"e' stato trovato in posizione "<<posto+1<<endl;
	else
	cout<<"L'elemento "<<elemento<<"non e' stato trovato."<<endl;

// ----RICERCA SEQUENZIALE SECOND FUNCTION-----	
	trovato=ricerca_sequenziale2(vett, dim, elemento, posto);
if(trovato)
	cout<<"L'elemento "<<elemento<<"e' stato trovato in posizione "<<posto+1<<endl;
	else
	cout<<"L'elemento "<<elemento<<"non e' stato trovato."<<endl;
...
//   ------SOTISTUZIONE ELEMETO-------
	cout<<"\n***SOSTITUZIONE ELEMENTO***\n"<<endl;
	cout<<"Inserisci l'elemento da sostituire: ";
	cin>>elemento;
	cout<<"Inserisci l'elemetocon cui sostituirlo "<<endl;
	cin>>e2;
	sostituisci(vett, dim, elemento, e2);
	stampa(vett, dim);

// -------ELIMINO DUPLICATI---------
	elimina_duplicati(vett, dim);
	stampa(vett, dim);

		system("PAUSE");
		system("CLS");
		cout<<"Voi riesegure il programma?[s/n]";
		cin>>risp;
		}while(risp=='s'||risp=='S');
